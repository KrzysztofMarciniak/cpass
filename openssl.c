...
static int derive_key(const char* password, unsigned char* salt,
                      unsigned char* key) {
...
int encrypt_file(const char* filename, const unsigned char* plaintext,
        ...
        if (!EVP_EncryptUpdate(ctx, ciphertext, &outlen, plaintext, (int)len))
                return 0;
...
// --- decrypt file ---
unsigned char* decrypt_file(const char* filename, size_t* outlen,
                            const char* password) {
        ...
        int outlen_tmp;
        if (!EVP_DecryptUpdate(ctx, plaintext, &outlen_tmp, ciphertext,
                               (int)cipherlen))
                return NULL;
