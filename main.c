...
// Each entry: [uint32 name_len][uint32 user_len][uint32
// pass_len][name][username][password]
static unsigned char* serialize_db(DB* db, size_t* outlen) {
        size_t total = 0;
        for (size_t i = 0; i < db->count; i++) {
                total += 12 + strlen(db->entries[i].name) +
                         strlen(db->entries[i].username) +
                         strlen(db->entries[i].password);
        }
        ...
        for (size_t i = 0; i < db->count; i++) {
                ...
                memcpy(p, db->entries[i].name, nl);
                p += nl;
                memcpy(p, db->entries[i].username, ul);
                p += ul;
                memcpy(p, db->entries[i].password, pl);
                p += pl;
        }
        *outlen = total;
        return buf;
...
static DB* deserialize_db(unsigned char* buf, size_t len) {
        ...
        while (p + 12 <= end) {
                ...
                db->entries =
                    realloc(db->entries, sizeof(Entry) * (db->count + 1));
                db->entries[db->count].name = strndup((char*)p, nl);
                p += nl;
                db->entries[db->count].username = strndup((char*)p, ul);
                p += ul;
                db->entries[db->count].password = strndup((char*)p, pl);
                p += pl;
...
static DB* load_db(const char* password) {
        ...
        if (!plaintext) {
                fprintf(stderr,
                        "Decryption failed. Wrong password or corrupt DB.\n");
                return NULL;
        }
...
static void cmd_edit(char* name) {
        ...
        for (size_t i = 0; i < db->count; i++) {
                if (strcmp(db->entries[i].name, name) == 0) {
                        ...
                        if (strlen(user) > 0) {
                                free(db->entries[i].username);
                                db->entries[i].username = strdup(user);
                        }
                        if (strlen(passw) > 0) {
                                free(db->entries[i].password);
                                db->entries[i].password = strdup(passw);
                        }
                        if (!save_db(db, pass))
                                fprintf(stderr, "Failed to save DB\n");
                        free_db(db);
                        printf("Updated '%s'\n", name);
                        return;
                }
...
int main(int argc, char** argv) {
        ...
        if (strcmp(argv[1], "init") == 0)
                ...
                cmd_ls(argc > 2 ? argv[2] : NULL);
        else if (strcmp(argv[1], "a") == 0 && argc == 5)
